...
@Component
@Repository
public interface ISymptomsRepository extends CrudRepository<SymptomsEntity,Integer>  {
  
    @Query("SELECT t FROM SymptomsEntity t WHERE t.job_id = ?1")
    List<SymptomsEntity> findSymptomsByJobId(String jobId);
    
    @Query("delete FROM SymptomsEntity t WHERE t.job_id = ?1")
    void deleteSymptomsByJobId(String jobId);
